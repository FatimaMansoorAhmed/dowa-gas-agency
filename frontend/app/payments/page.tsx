...
// Normalizes the two sources that actually move a customer's money to a
// destination — standalone /payment-receipts rows, and approved Unified
// Sale batches (which never create a Payment row; their settlement posts
// straight to balances on approval, see unified_sale.approve_unified_sale)
// — into one shape the register can render without knowing which source a
// row came from.
type RegisterRow = {
  id: string;
  display_id: string;
...
function PaymentsBody() {
  ...
  // Merge the two sources of "money routed to a destination" into one
  // register: standalone /payment-receipts rows, and approved Unified Sale
  ...
  const filteredRegister = useMemo(() => {
    return registerRows.filter((row) => {
      ...
      // Exact Route Matching Logic Fix
...
return (
    <div className="max-w-[1700px] mx-auto w-full space-y-6 px-4 sm:px-6">
      ...
      {/* MAIN TWO-COLUMN LAYOUT - BALANCED FORM & FULL-WIDTH REGISTER */}
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-6 items-start w-full">
        ...
        {/* RIGHT REGISTER TABLE (8 COLUMNS - FULLY STRETCHED) */}
