...
export default function ShopStockBatchesPanel({
  ...
  // Collapsed by default (§ Shop Detail visual redesign) — a detail view,
  // not something that needs to be visible on every page load. Still
  // renders in full for print, matching the Correction History pattern
  // this mirrors (hidden print:block on the collapsed content).
    ...
    <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
      ...
      >
        ...
        <span className="shrink-0 text-xs font-semibold text-teal">
          {expanded ? t("shopDetail.hideDetails") : t("shopDetail.showDetails")}
        </span>
      </button>

      {/* Printed in full regardless of expanded state (hidden print:block),
          same convention as Correction History further down the page. */}
      <div className={expanded ? "" : "hidden print:block"}>

        <div className="flex flex-col gap-4 border-b border-slate-200 bg-white px-6 py-5 lg:flex-row lg:items-center lg:justify-end">
